#!/usr/bin/env python3
"""Generate a blank input-workbook template for elisa_analysis.

Usage:
    python scripts/make_template.py [output_path]
"""
import sys
...
def build_template() -> Workbook:
    ...
    _write_grid(ws_raw, "Well")
    ws_raw["A11"] = "Paste your plate reader's 8x12 OD/absorbance grid above (one value per well)."
    ws_raw["A11"].font = Font(italic=True, size=9)

    ws_layout = wb.create_sheet("Plate Layout")

    def default_layout(row_letter, col_num):
        # Columns 1-2: standards STD1 (top, A) .. STD8 (bottom, H), duplicated in col 1 & 2.
        if col_num in (1, 2):
    ...
    ws_layout["A11"] = (
        "Label each well: STDn for standards (must match the Standards sheet), "
        "BLANK for zero/blank wells, and any short code (e.g. UNK1) for sample wells. "
        "Repeat a label across wells for replicates."
    )
...
if __name__ == "__main__":
    out_path = sys.argv[1] if len(sys.argv) > 1 else "elisa_template.xlsx"
